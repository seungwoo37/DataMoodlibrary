...
class MorphSentimentAnalyzer:
    ...
    def text_analyze(self, text):
        ...
        전체 감성 점수는 다음 단계를 거쳐 계산됩니다.

        1. **형태소 분석**  
           텍스트를 토큰화하고 어간 추출(`stem=True`)을 수행합니다.

        2. **기본 점수 계산**  
           감성 사전을 참조하여 초기 점수를 산출합니다.

        3. **TF-IDF 가중치 적용**  
           단어 빈도(TF)를 로그 스케일로 보정하고, 사전 정의된 IDF 값을 적용합니다.
        ...
        4. **문맥 규칙 적용**
           ...
           * **부정어 처리** — 앞뒤 3개 토큰 내 부정어 존재 시 점수 부호 반전  
             (예: ``안``, ``못``, ``아니다`` 등)
           ...
           * **감성 전환(Contrast) 처리**  
             접속사(예: ``하지만``, ``그러나``) 이후의 감성어에 가중치 x1.3 적용

        5. **문장 길이 보정**  
           짧은 문장은 감성 단어 영향력 증가(1.2), 긴 문장은 감소(0.9).

        6. **정규화**  
           전체 점수를 최소·최대 점수 범위 내에서 0~1 사이로 정규화하고, 0~100 백분율로 변환합니다.
        ...
        dict
            ...
            * ``score`` – 최종 점수  
            * ``percentage`` – 백분율  
            * ``num_sentiment_words`` – 감성 관련 단어 수  
            * ``total_words`` – 전체 토큰 수  
            * ``reason`` – 단어별 계산 과정 설명 리스트
        ...
        .. code-block:: python
           ...
           analyzer.text_analyze("서비스가 별로 좋지 않았습니다")

        위 입력의 출력 예시는 다음과 같습니다 (요약된 형태).
